...
sD=som_read_data(strcat('cell',user_no{index},char{index2},user_count{index3},'t','.data'));

% There are three if condition for determine log & logistic values

if char{index2}=='all'
sD=som_normalize(sD,'log',[1 2 3 5 6]); % for greater than 10
sD=som_normalize(sD,'logistic',[4]); % for less than 10
end
if char{index2}=='a'
...
if char{index2}=='b'
sD=som_normalize(sD,'log',[1 2 4 5]); 
sD=som_normalize(sD,'logistic',[3]);
end
if char{index2}=='c'
...
h1 = som_hits(sM,sD.data(1:6840,:)); 
h2 = som_hits(sM,sD.data(6841:13680,:));
...
sDt=som_read_data(strcat('cell',user_no{index},char{index2},'tt','.data'));

if char{index2}=='all'
sDt=som_normalize(sDt,'log',[1 2 3 5 6]); % for greater than 10
sDt=som_normalize(sDt,'logistic',[4]); % for less than 10
end
if char{index2}=='a'
...
if char{index2}=='b'
sDt=som_normalize(sDt,'log',[1 2 4 5]); 
sDt=som_normalize(sDt,'logistic',[3]);
end
if char{index2}=='c'
...
h3 = som_hits(sM,sDt.data(1:2000,:)); %1 den 200 evetler
h4 = som_hits(sM,sDt.data(2001:40000,:)); % 201 den 2000 e hayýrlar
...
saveas(gcf,strcat('cell',user_no{index},user_count{index3},char{index2}),'jpg');
close
end
end
end