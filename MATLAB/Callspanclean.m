%Mert Pehlivancik
%Callspanclean Som Images


char={'all','a','b','c','d','e'}; % define string for cellspan column 
...
sD=som_read_data(strcat('call',user_no{index},char{index2},user_count{index3},'t','.data'));

% There are three if condition for determine log & logistic values

if char{index2}=='all'
sD=som_normalize(sD,'log',[1 2 3 8 9]); % for greater than 10
sD=som_normalize(sD,'logistic',[4 5 6 7]); % for less than 10
end
if char{index2}=='a'
sD=som_normalize(sD,'log',[1 2 3 7 8]); % for greater than 10
sD=som_normalize(sD,'logistic',[4 5 6]); % for less than 10
end
if char{index2}=='b'
sD=som_normalize(sD,'log',[1 2 3 7 8]); 
sD=som_normalize(sD,'logistic',[4 5 6]);
end
if char{index2}=='c'
sD=som_normalize(sD,'log',[1 2 7 8]); 
sD=som_normalize(sD,'logistic',[3 4 5 6]);
end
if char{index2}=='d'
sD=som_normalize(sD,'log',[1 2 7 8]); 
sD=som_normalize(sD,'logistic',[3 4 5 6]);
...
h1 = som_hits(sM,sD.data(1:285,:)); 
h2 = som_hits(sM,sD.data(286:570,:));
...
sDt=som_read_data(strcat('call',user_no{index},char{index2},'tt','.data'));

if char{index2}=='all'
sDt=som_normalize(sDt,'log',[1 2 3 8 9]); % for greater than 10
sDt=som_normalize(sDt,'logistic',[4 5 6 7]); % for less than 10
end
if char{index2}=='a'
sDt=som_normalize(sDt,'log',[1 2 3 7 8]); % for greater than 10
sDt=som_normalize(sDt,'logistic',[4 5 6]); % for less than 10
end
if char{index2}=='b'
sDt=som_normalize(sDt,'log',[1 2 3 7 8]); 
sDt=som_normalize(sDt,'logistic',[4 5 6]);
end
if char{index2}=='c'
sDt=som_normalize(sDt,'log',[1 2 7 8]); 
sDt=som_normalize(sDt,'logistic',[3 4 5 6]);
end
if char{index2}=='d'
sDt=som_normalize(sDt,'log',[1 2 7 8]); 
sDt=som_normalize(sDt,'logistic',[3 4 5 6]);
...
h3 = som_hits(sM,sDt.data(1:80,:)); %1 den 200 evetler
h4 = som_hits(sM,sDt.data(81:1600,:)); % 201 den 2000 e hayýrlar
...
saveas(gcf,strcat('call',user_no{index},user_count{index3},char{index2}),'jpg');
close
end
end
end