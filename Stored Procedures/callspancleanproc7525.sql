CREATE DEFINER=`root`@`localhost` PROCEDURE `callspancleanproc7525`()
BEGIN
DECLARE count INT Default 0;
  simple_loop: LOOP
    ...
    SET @statement = CONCAT('insert into tmptbl(countofrow) select COUNT(*) from callspanclean3_',@b,'');
    ...
    SET @statement2 = CONCAT('Create table callspanclean3_',@b,'a ( SELECT * FROM callspanclean3_',@b,' ORDER BY starttime LIMIT ',@percentageof75,');');             
         PREPARE stmt2 FROM @statement2;
                 EXECUTE stmt2;
                 DEALLOCATE PREPARE stmt2;
     SET @statement3 = CONCAT('Create table callspanclean3_',@b,'b ( SELECT * FROM callspanclean3_',@b,' ORDER BY starttime DESC LIMIT ',@percentageof25,');');             
