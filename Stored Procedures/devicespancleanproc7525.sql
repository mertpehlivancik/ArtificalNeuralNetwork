CREATE DEFINER=`root`@`localhost` PROCEDURE `devicespancleanproc7525`()
BEGIN
DECLARE count INT Default 0;
  simple_loop: LOOP
    ...
    SET @statement = CONCAT('insert into tmptbl(countofrow) select COUNT(*) from devicespanclean2_',@b,'');
    ...
    SET @statement2 = CONCAT('Create table devicespanclean2_',@b,'a ( SELECT * FROM devicespanclean2_',@b,' ORDER BY starttime LIMIT ',@percentageof75,');');             
         PREPARE stmt2 FROM @statement2;
                 EXECUTE stmt2;
                 DEALLOCATE PREPARE stmt2;
     SET @statement3 = CONCAT('Create table devicespanclean2_',@b,'b ( SELECT * FROM devicespanclean2_',@b,' ORDER BY starttime DESC LIMIT ',@percentageof25,');');             
