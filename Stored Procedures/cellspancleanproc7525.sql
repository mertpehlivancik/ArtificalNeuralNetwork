CREATE DEFINER=`root`@`localhost` PROCEDURE `cellspancleanproc7525`()
BEGIN
DECLARE count INT Default 0;
  simple_loop: LOOP
    ...
    SET @statement = CONCAT('insert into tmptbl(countofrow) select COUNT(*) from cellspanclean2_',@b,'');
    ...
    SET @statement2 = CONCAT('Create table cellspanclean2_',@b,'a ( SELECT * FROM cellspanclean2_',@b,' ORDER BY starttime LIMIT ',@percentageof75,');');             
         PREPARE stmt2 FROM @statement2;
                 EXECUTE stmt2;
                 DEALLOCATE PREPARE stmt2;
     SET @statement3 = CONCAT('Create table cellspanclean2_',@b,'b ( SELECT * FROM cellspanclean2_',@b,' ORDER BY starttime DESC LIMIT ',@percentageof25,');');             
